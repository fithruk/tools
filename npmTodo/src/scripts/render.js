...
function listItemElemHandle(e) {
  if (e.target.closest('.list__item-checkbox')) {
    getApartTask(e.target.dataset.id).then((data) => updateExsistTask(
    ...
    removeTaskFromData(e.target.parentNode.children[0].dataset.id).then(() => getData().then((data) => renderTasks(data)));
  }
}
